...
namespace CryptoKnight.Client.Plugin.RC2
{
    public sealed class Plugin : MarshalByRefObject, IPlugin
    {
        public string Name => "RC2";

        private static readonly byte[] Salt = Encoding.ASCII.GetBytes("rats can't jump");
        ...
        public byte[] Encrypt(string data, string password)
        {
            try
            {
                ...
                using (var cryptoAlgorithm = SysCrypto.RC2.Create())
                {
