...
namespace CryptoKnight.Client.Plugin.Rijindael
{
    public sealed class Plugin : MarshalByRefObject, IPlugin
    {
        public string Name => "Rijindael";

        private static readonly byte[] Salt = Encoding.ASCII.GetBytes("carrots are blue");
        ...
        public byte[] Encrypt(string data, string password)
        {
            try
            {
                ...
                using (var cryptoAlgorithm = SysCrypto.Rijndael.Create())
                {
